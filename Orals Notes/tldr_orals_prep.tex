...
\begin{table}[h!]
	...
	\begin{tabular}{|>{\centering\arraybackslash}m{4.2cm}|m{7.5cm}|}
		...
		\hline 
		\multirow{2}{*}{Harmonic Analysis} & Three-Lines Lemma \newline Riesz Interpolation Theorem \newline Hausdorff-Young Inequality \\ \arrayrulecolor{black!25}\cline{2-2}\arrayrulecolor{black} & Extension of Fourier Transform  (Bounded Map from $L^{p} \to L^{q}$ for all $p \in [1,2]$) \\ \arrayrulecolor{black!25}\cline{2-2}\arrayrulecolor{black}  & Young's Inequality for Convolutions \newline Hilbert Transform \\\hline 
	\end{tabular}
\end{table}
...
\begin{itemize}
	\item (\textbf{Three-Lines Lemma}) First interpolation result we obtain. For a holomorphic function $\Phi$ on the strip $S = \{0 < \operatorname{Re}(z) < 1\}$ and continuous on $\bar{S}$, and bounded by $M_{0}$ and $M_{1}$ on either side of the strip, $\Phi$ is also bounded on any vertical line between $\operatorname{Re}(z) = 0$ and $\operatorname{Re}(z) = 1$: 
	\begin{equation*}
		\sup_{y \in \mathbb{R}}|\Phi(t + iy)| \leq M_{0}^{1 - t}M_{1}^{t}, \qquad \forall t \in [0,1]. 
	\end{equation*} 
	
	\item (\textbf{Riesz-Thorin Interpolation Theorem}) Consider a linear operator $T$ that is bounded from $L^{p_{0}} \to L^{q_{0}}$ and from $L^{q_{0}}$ to $L^{q_{1}}$. Then this theorem gives us the conditions under which $T$ is a bounded operator from $L^{p}$ to $L^{q}$. In particular, one has that $T$ is a bounded operator from $L^{p}$ to $L^{q}$ for all pairs $(p, q)$ such that 
	\begin{equation*}
		\frac{1}{p} = \frac{1 - t}{p_{0}} + \frac{t}{p_{1}}, \qquad \frac{1}{q} = \frac{1 - t}{q_{0}} + \frac{t}{q_{1}}. 
	\end{equation*} 
	\textbf{Key Facts:} (1) Reduces the effort to establishing endpoint estimates. (2) applications include the \textbf{Hausdorff-Young inequality}, \textbf{extension of the Fourier transforrm}, \textbf{Young's inequality for convolutions}. 
\end{itemize}
...
\begin{itemize}
	...
	\item (\textbf{Bootstrap Argument}) A method of establishing certain desired results. The idea is that if we wish to show that some result $A$ is true, we start by assuming that some marginally better assumption $A_{\epsilon}$ is true, and showing that this better result forces the original result. 
