...
\begin{prb}{2.1}
	...
	\begin{quote}
		\textbf{Theorem 3.4} (Uniform Decay Rate)\textbf{.} \textit{For $(\phi_0, \phi_1) \in C_c^{\infty}(\mathbb{R}^n) \times C_c^{\infty}(\mathbb{R}^n)$, and $\phi$ the unique solution to the homogeneous initial value problem, there exists $C = C(\phi_0, \phi_1) > 0$ so that}
		\begin{equation*}
			\sup_{x \in \mathbb{R}^n}|\phi(t,x)| \leq \frac{C}{(1 + t)^{\frac{n-1}{2}}}
		\end{equation*}
		\textit{for $t\geq 0$.}
	\end{quote}
	Show that using the same proof as for the $n = 3$ case for even $n$ gives the weaker decay 
	\begin{equation*}
		\sup_{x \in \mathbb{R}^{n}}|\phi(t, x)| \leq \frac{C}{(1 + t)^{\frac{n-2}{2}}}. 
	\end{equation*} 
\end{prb}
\end{document} 
